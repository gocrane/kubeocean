...
Tapestry Manager is the central control plane component of the Tapestry system that manages cluster bindings and resource leasing policies.
...
./tapestry-manager \
  --leader-elect=true \
  --leader-election-id=tapestry-manager-leader \
...
- `tapestry_cluster_bindings_total`: Total number of cluster bindings by phase
- `tapestry_leader_election_status`: Leader election status (1 for leader, 0 for follower)
- `tapestry_sync_duration_seconds`: Duration of synchronization operations
- `tapestry_sync_errors_total`: Total number of synchronization errors
...
The Tapestry Manager requires the following permissions:
...
```bash
kubectl get lease tapestry-manager-leader -n tapestry-system
```

View controller events:
```bash
kubectl get events -n tapestry-system --field-selector involvedObject.kind=ClusterBinding
```
