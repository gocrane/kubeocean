...
type TokenManagerInterface interface {
	GetServiceAccountToken(namespace, name string, tr *authenticationv1.TokenRequest) (*authenticationv1.TokenRequest, error)
	DeleteServiceAccountToken(podUID types.UID)
}

// NewManager returns a new token manager.
func NewManager(c clientset.Interface, logger logr.Logger) TokenManagerInterface {
...
func (m *Manager) GetServiceAccountToken(namespace, name string, tr *authenticationv1.TokenRequest) (*authenticationv1.TokenRequest, error) {
	...
	if err != nil {
		...
		default:
			m.logger.Error(err, "Couldn't update token", "cacheKey", key)
			return ctr, nil
		}
	}
...
// TODO: need to optimize this used podUID mapping to cache
...
func (m *Manager) requiresRefresh(_ context.Context, tr *authenticationv1.TokenRequest) bool {
	if tr.Spec.ExpirationSeconds == nil {
		cpy := tr.DeepCopy()
		cpy.Status.Token = ""
		m.logger.Error(nil, "Expiration seconds was nil for token request", "tokenRequest", cpy)
		return false
	}
	now := m.clock.Now()
