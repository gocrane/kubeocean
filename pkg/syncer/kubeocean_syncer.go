...
import (
	...
	"github.com/TKEColocation/kubeocean/pkg/syncer/clusterbinding"
...
func NewKubeoceanSyncer(mgr manager.Manager, client client.Client, scheme *runtime.Scheme, bindingName string, physicalClientQPS int, physicalClientBurst int) (*KubeoceanSyncer, error) {
	log := ctrl.Log.WithName("kubeocean-syncer").WithValues("binding", bindingName)

	return &KubeoceanSyncer{
		Client:              client,
		Scheme:              scheme,
		Log:                 log,
...
func (ts *KubeoceanSyncer) Start(ctx context.Context) error {
	ts.Log.Info("Starting Kubeocean Syncer")
	...
	ts.Log.Info("Kubeocean Syncer started successfully")
	...
	// Graceful shutdown
	ts.Log.Info("Kubeocean Syncer stopping")
	ts.shutdown(ctx)
	<-ts.phyMgrCh
...
func (ts *KubeoceanSyncer) loadClusterBinding(ctx context.Context) error {
...
func (ts *KubeoceanSyncer) GetClusterBinding() *cloudv1beta1.ClusterBinding {
	return ts.clusterBinding
}

// setupPhysicalClusterConnection sets up connection to the physical cluster with retry mechanism
func (ts *KubeoceanSyncer) setupPhysicalClusterConnection(ctx context.Context) error {
...
func (ts *KubeoceanSyncer) readKubeconfigSecret(ctx context.Context) ([]byte, error) {
...
func (ts *KubeoceanSyncer) testPhysicalClusterConnectivityWithTimeout(ctx context.Context, kubeClient kubernetes.Interface) error {
...
func (ts *KubeoceanSyncer) initializeSyncers() error {
...
func (ts *KubeoceanSyncer) startSyncLoop(ctx context.Context) {
...
func (ts *KubeoceanSyncer) shutdown(_ context.Context) {
	ts.Log.Info("Shutting down Kubeocean Syncer")
	...
	ts.Log.Info("Kubeocean Syncer shutdown completed")
}

// Stop stops the Kubeocean Syncer
func (ts *KubeoceanSyncer) Stop() {
	close(ts.stopCh)
}

// setupClusterBindingController sets up the ClusterBinding controller
func (ts *KubeoceanSyncer) setupClusterBindingController(_ context.Context) error {
